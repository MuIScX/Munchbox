            ...
            <div className="bg-white rounded-[2rem] shadow-sm border border-slate-200 overflow-hidden flex flex-col h-full">
              ...
              <div className="p-8 flex-1 overflow-y-auto">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="font-bold text-slate-800 text-2xl italic flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {ingredients.map((ing, i) => {
                    return (
                      <div key={i} className="flex justify-between items-center bg-slate-50/50 p-5 rounded-2xl border border-slate-100">
                        <div className="flex items-center gap-3">
                          <div className="w-3 h-3 rounded-full shadow-sm bg-orange-400 shrink-0" />
                          <div>
                            <p className="font-bold text-slate-700 text-base leading-none mb-1">{ing.ingredient_name}</p>
                            <p className="text-xs font-normal text-slate-300 uppercase tracking-widest">Requirement</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <span className="text-xl font-black text-slate-800">{ing.amount}</span>
                          <span className="text-xs ml-1 font-normal text-slate-400 lowercase">{ing.unit}</span>
                        </div>
                      </div>
                    );
                  })}
